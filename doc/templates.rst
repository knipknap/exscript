...
can be used to retrieve the response of the ls -l and df commands from a
unix host::
...
Note that the -d switch allows passing variables into the *Exscript* .
The example executes the command ls -l .profile. You can also assign a
value to a variable within an *Exscript*::
...
*Exscript* Built-in Commands
----------------------------

By default, any content of an *Exscript* is sent to the remote host.
...
The following *Exscript* executes the “ls” command only if “ls -l
.profile” did not produce a result:
...
following *Exscript* executes the “ls” command three times:
...
The following *Exscript* uses the ls command to show the content of a
...
For a list of supported functions please refer to appendix [stdlib].
...
Whenever such an error is detected, the currently running *Exscript*
