...
class SSH2(Protocol):
    ...
    def _domatch(self, prompt, flush):
        ...
        while not self.cancel:
            ...
            search_window = self.buffer.tail(search_window_size).decode('utf-8')
            ...
            if flush:
                self.response = self.buffer.pop(end).decode('utf-8')
            else:
                self.response = self.buffer.head(end).decode('utf-8')
            return n, match
